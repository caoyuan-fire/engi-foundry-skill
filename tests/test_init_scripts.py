...
class InitScriptsTests(unittest.TestCase):
    ...
    def test_directory_config_script_creates_empty_and_filled_formal_files(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("<package-root>/phase.index.json", paths)
            self.assertIn("<package-root>/ROADMAP.md", paths)
            self.assertIn("<package-root>/PHASE-001/phase.config.json", paths)
