...
当用户要求开始实现某个功能且当前没有任务包时，EngiFoundry 先按风险和清晰度分类，再编辑代码。范围明确、低风险的小实现可以走 ad-hoc，并按工程纪律执行，包括可行时优先 test-first。范围大、高风险、多步骤、跨模块、需要交接或需求含糊的实现请求，应自动先进入 package planning；EngiFoundry 不应要求用户把“先编制任务包”作为单独前置操作，也不应在任务包契约 ready 前直接按 TDD 开始实现，除非缺失信息必须先由用户澄清。这个 package planning 步骤不是默认的用户批准暂停点；任务包 ready 后应继续进入执行，除非用户要求批准、任务包要求人工批准，或仍存在具体 blocker。
